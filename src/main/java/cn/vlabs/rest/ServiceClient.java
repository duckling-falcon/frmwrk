...
public class ServiceClient {
	...
	public Object sendService(String service, Object message)
    ...
    //
	// test if this method is in use <2021-12-30 Thu>
	// 
	...
	private void sendService(String service, PostMethod method,
                             IReader reader) throws ServiceException {
		try {
	...
	private PostMethod createMethod(String service, Object message) {
		...
		EncodablePostMethod method = new EncodablePostMethod(
            context.getServerURL(), context.getCharset());
		method.addParameter("RequestXML", toXML(env));
		method.setDoAuthentication(true);
		return method;
	}

	private PostMethod createMultiPart(String service,
                                       Object message,
                                       IResource stream) {
		Envelope env = createRequest(service, message, true);
		EncodablePostMethod method = new EncodablePostMethod(
            context.getServerURL(), context.getCharset());
		FilePart xml = new FilePart(
            "RequestXML", new StringBufferPartSource(toXML(env)));
	...
	private Envelope createRequest(
        String service, Object message, boolean hasStream) {
